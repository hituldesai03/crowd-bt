#!/bin/bash
# Distributed training launcher script for Crowd-BT Quality Scorer
#
# Usage:
#   ./train_distributed.sh <num_gpus> [additional args...]
#
# Example:
#   ./train_distributed.sh 4 --epochs 50 --batch-size 16
#   ./train_distributed.sh 2 --data-source s3 --backbone efficientnet_b5

# Check if number of GPUs is provided
if [ -z "$1" ]; then
    echo "Error: Number of GPUs not specified"
    echo "Usage: $0 <num_gpus> [additional args...]"
    echo "Example: $0 4 --epochs 50 --batch-size 16"
    exit 1
fi
